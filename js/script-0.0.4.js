...
function lt3aRequest() {
  if (requestPHP.lt3a == false) {
    requestPHP.lt3a = true;

    let ajaxRequest = new XMLHttpRequest();

    ajaxRequest.onreadystatechange = function () {
      if (ajaxRequest.readyState == 4) {
        requestPHP.lt3a = false;

        const status = document.querySelector("#status-lt3a");
        const dataEsp = document.querySelector("#data-lt3a");
        const idEsp = document.querySelector("#id-lt3a");
    ...
    ajaxRequest.open("GET", urlLt3a, true);
    ajaxRequest.send(null);
  }
}
